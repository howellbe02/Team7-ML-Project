...
for (weight in unique_weights) {
  ...
  data_subset <- data_subset[
    data_subset$`Match Length Sec` >= 60 &
      !is.na(data_subset$`Standing Head Shots Attempted`) &
      !is.na(data_subset$`Standing Head Shots Landed`) &
      data_subset$`Standing Head Shots Attempted` > 0 &
      data_subset$`Standing Head Shots Landed` <= data_subset$`Standing Head Shots Attempted`, ]
  ...
  model <- bayesglm(
    cbind(data_subset$`Standing Head Shots Landed`,
          data_subset$`Standing Head Shots Attempted` - data_subset$`Standing Head Shots Landed`) ~ 
  ...
  write_xlsx(offense_df, paste0(folder, "Offense_Standing_Head_Accuracy.xlsx"))
  write_xlsx(defense_df, paste0(folder, "Defense_Standing_Head_Accuracy.xlsx"))
}
