...
for (weight in unique_weights) {
  ...
  data_subset <- data_subset[
    data_subset$`Match Length Sec` >= 60 &
      !is.na(data_subset$`Takedown Attempts`) &
      !is.na(data_subset$Takedowns) &
      data_subset$`Takedown Attempts` > 0 &
      data_subset$Takedowns <= data_subset$`Takedown Attempts`, ]
  ...
  model <- bayesglm(
    cbind(data_subset$Takedowns,
          data_subset$`Takedown Attempts` - data_subset$Takedowns) ~ 
  ...
  write_xlsx(offense_df, paste0(folder, "Offense_Takedown_Accuracy.xlsx"))
  write_xlsx(defense_df, paste0(folder, "Defense_Takedown_Accuracy.xlsx"))
}