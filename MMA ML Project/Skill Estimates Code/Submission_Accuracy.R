...
for (weight in unique_weights) {
  ...
  data_subset <- data_subset[
    data_subset$`Match Length Sec` >= 60 &
      !is.na(data_subset$`Submission Attempts`) &
      !is.na(data_subset$Submission) &
      data_subset$`Submission Attempts` > 0 &
      data_subset$Submission <= data_subset$`Submission Attempts`, ]
  ...
  model <- bayesglm(
    cbind(data_subset$Submission,
          data_subset$`Submission Attempts` - data_subset$Submission) ~ 
  ...
  write_xlsx(offense_df, paste0(folder, "Offense_Submission_Accuracy.xlsx"))
  write_xlsx(defense_df, paste0(folder, "Defense_Submission_Accuracy.xlsx"))
}
