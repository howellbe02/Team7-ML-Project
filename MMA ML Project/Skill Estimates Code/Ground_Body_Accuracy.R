...
for (weight in unique_weights) {
  ...
  data_subset <- data_subset[
    data_subset$`Match Length Sec` >= 60 &
      !is.na(data_subset$`Ground Body Shots Attempted`) &
      !is.na(data_subset$`Ground Body Shots Landed`) &
      data_subset$`Ground Body Shots Attempted` > 0 &
      data_subset$`Ground Body Shots Landed` <= data_subset$`Ground Body Shots Attempted`, ]
  ...
  model <- bayesglm(
    cbind(data_subset$`Ground Body Shots Landed`,
          data_subset$`Ground Body Shots Attempted` - data_subset$`Ground Body Shots Landed`) ~ 
  ...
  write_xlsx(offense_df, paste0(folder, "Offense_Ground_Body_Accuracy.xlsx"))
  write_xlsx(defense_df, paste0(folder, "Defense_Ground_Body_Accuracy.xlsx"))
}
