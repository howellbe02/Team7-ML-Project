...
df$Knockout_Or_Knockdown <- df$Knockout + df$Knockdowns
...
for (weight in unique_weights) {
  ...
  data_subset <- data_subset[
    data_subset$`Match Length Sec` >= 60 &
      !is.na(data_subset$`Standing Head Shots Landed`) &
      !is.na(data_subset$Knockout_Or_Knockdown) &
      data_subset$`Standing Head Shots Landed` > 0 &
      data_subset$Knockout_Or_Knockdown <= data_subset$`Standing Head Shots Landed`, ]
  ...
  model <- bayesglm(
    cbind(data_subset$Knockout_Or_Knockdown,
          data_subset$`Standing Head Shots Landed` - data_subset$Knockout_Or_Knockdown) ~ 
  ...
  write_xlsx(offense_df, paste0(folder, "Offense_Knockout_Probability.xlsx"))
  write_xlsx(defense_df, paste0(folder, "Defense_Knockout_Probability.xlsx"))
}